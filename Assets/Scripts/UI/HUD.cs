...
public class HUD : MonoBehaviour
{
	...
	private float bossHealthBarWidthEased;
	...
	void Start()
	{
		...
		bossHealthBarWidth = 1;
		bossHealthBarWidthEased = bossHealthBarWidth;
		ammoBarWidth = 1;
	...
	void Update()
	{
		...
		//Controls the width of the Boss health bar based on the boss's total health
		if (bossHealthBar != null)
		{
			...
			if (boss != null)
			{
				EnemyBase healthScript = boss.GetComponent<EnemyBase>();
				bossHealthBarWidth = (float)healthScript.getHealth()[0] / (float)healthScript.getHealth()[1];
				bossHealthBarWidthEased += (bossHealthBarWidth - bossHealthBarWidthEased) * Time.deltaTime * bossHealthBarWidthEased;
				bossHealthBar.transform.localScale = new Vector2(bossHealthBarWidthEased, 1);
			}
			else
			{
				Destroy(bossHealthBar.transform.parent.gameObject);
			}
		}
